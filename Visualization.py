...
#     ax.set_ylabel('depth')
#     #ax.set_zlabel('Z')
#     #ax.set_xlim(max_crit_pt+padding, min_crit_pt-padding) # reversed #ax.set_ylim(0, depth_padding*landscape.max_depth+1)
...
# #     # x-axis for grid
# #     domain = np.linspace(landscape.start-padding*0.1, landscape.stop+padding*0.1, num=num_step
# #     # for each landscape function
# #     for depth, l in enumerate(landscape):
# #         # sequential pairs in landscape
# #         # xs, zs = zip(*l)
# #         image = np.interp(domain, np.linspace(start=landscape.start, stop=landscape.stop, num=landscape.num_dims), l)
# #         for x, z in zip(domain,image):
# #             if z == 0.:
# #                 # plot a single point here?
# #                 continue # moves to the next iterable in for loop
